...
void drawObstacle(float roadSpeed, GLuint texturePolice1, GLuint texturePolice2, GLuint texturePolice3, GLuint textureBubble) {
    ...
    // Pour les gyrophares de la voiture de police (obstacle)
    ...
    if (_isAlreadyObstacle) {
        glPushMatrix();
            ...
            if (_obstacle.isBubbled) {
                ...
                glBegin(GL_QUADS);
                    glTexCoord2f(1, 1); glVertex2f(-0.2, -0.2);
                    glTexCoord2f(0, 1); glVertex2f(-0.2, 0.2);
                    glTexCoord2f(0, 0); glVertex2f(0.2, 0.2);
                    glTexCoord2f(1, 0); glVertex2f(0.2, -0.2);
                glEnd();
            } else {
                glTranslatef(_obstacle.position.x, _obstacle.position.y, 0);
                glBegin(GL_QUADS);
                    glTexCoord2f(1, 1); glVertex2f(-0.2, -0.1);
                    glTexCoord2f(0, 1); glVertex2f(-0.2, 0.1);
                    glTexCoord2f(0, 0); glVertex2f(0.2, 0.1);
                    glTexCoord2f(1, 0); glVertex2f(0.2, -0.1);
                glEnd();
            }
